...
printConfigFunc = ["has the highest", "has the lowest"]
...
rowTemplate = "SELECT CONCAT( b1.$SUB_PK$ , $PRINT_O$, b1.$A1$, $A1_NAME$ ) , b1.$SUB_PK$ , b2.$SUB_PK$ , b1.$A1$ , b2.$A1$ , b1.$PK$ , b2.$PK$ " \
              "FROM $TABLE$ b1 , $TABLE$ b2 " \
...
functionTemplate = "SELECT CONCAT( b1.$SUB_PK$, $PRINT_FUNC$, $A1_NAME$ ), b1.$SUB_PK$, b2.$SUB_PK$,  b1.$A1$, b2.$A1$ , b1.$PK$ , b2.$PK$ " \
                   "FROM $TABLE$ b1, $TABLE$ b2 " \
                   "WHERE b1.$SUB_PK$ = b2.$SUB_PK$ AND b1.$A1$ = (SELECT $FUNC$( $A1$ ) FROM $TABLE$) AND b2.$A1$ $MT_FUNC$ (SELECT $FUNC$( $A1$ ) FROM $TABLE$)"
